...
|  FEA_SPEC input by Rod Johnson.  Modified by Derek Lin so that	|
|  atan2: domain error when 0 complex number is encountered.		|
|									|
|  Module:  plotspec.c							|
|									|
...
static char *sccs_id = "@(#)plotspec.c	3.12	1/20/97	ESI/ERL";
...
/* Process command-line options */
    ...
    while ((c = getopt(argc, argv, "dlr:st:x:y:DEFX:Y:")) != EOF)
	switch (c)
	{
...
/* allocate memory */
    ...
    fea_skiprec(specfile, firstrec - 1, ih);
    ...
    if (!sflag)
	draw_box(xmink, xmaxk, xstepk, xdp, ymin, ymax, ystep, ydp);
