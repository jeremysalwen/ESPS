...
static char *sccs_id = "@(#)scatplot.c	3.8	1/27/97	ESI/ERL";
...
/* parse command line for parameters */
    ...
    while ((c = getopt(argc, argv, "e:r:s:t:x:H:V:X:Y:")) != EOF)
	switch (c) 
        {
	case 'e':
	    erange = optarg;
...
/* miscellaneous initializations and consistency checks */
    ...
    if (debug_level >= 1)
	pr_larray("element numbers (decremented)", 2*nfiles, elem_array);
...
/* copy to temp file if necessary. */
	...
	else
	{
	    ...
	    */
		...
		if (h[i_file]->common.type == FT_FEA)
				/* FEA file or non-ESPS converted to FEA */
		{
		    struct fea_data *tmprec; /* input data record */
		    struct header   *tmphd;

		    tmprec = allo_fea_rec(h[i_file]);
		    if (tmprec == NULL)
		    {
			Fprintf(stderr, "%s: can't allocate memory for %s.\n",
				ProgName, "temporary FEA record");
			exit(1);
		    }

		    write_header(copy_header(h[i_file]), tmpstrm);
		    rewind(tmpstrm);
		    tmphd = read_header(tmpstrm);
		    rewind(tmpstrm);

		    if (debug_level >= 2)
			Fprintf(stderr,
			    "Allocated space for FEA record.\n");
		    if (debug_level >= 3)
			Fprintf(stderr, "Reading records:");

		    nrec = 0;
		    while (get_fea_rec(tmprec, h[i_file], filep[i_file])
			   != EOF)
		    {
			put_fea_rec(tmprec, tmphd, tmpstrm);
			nrec++;
		    ...
		    {
			/* ndrec[i_file] == -1 but filep[i_file] != stdin.
			 * Probably file with variable record size.
			 */
		...
		else		/* obsolete ESPS file types */
		{
		    ...
		    {
			Fprintf(stderr,
				" variable record size not allowed in %s.\n",
				files[i_file]);
			exit(1);
		    }
...
/* read data into arrays, find extreme x and y values */
	...
	fea_skiprec(filep[i_file], start_p - 1, h[i_file]);
...
/* plot data */
	...
	plot_letter( xdata[i_file], ydata[i_file], (int) nans[i_file], 5,
...
/* print titles, etc. */
    ...
    {
	Printf("c 5\nm 375 %d\nd 375 %d\n", upperx, lowerx);
	(void) strncpy(txtbuf, command_line, COM_CHARS);
	txtbuf[COM_CHARS] = '\0';
