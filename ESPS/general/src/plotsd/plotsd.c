 ...
 * Revised by:
 *
 * Plot ESPS sample data
 *
 */

static char *sccs_id = "@(#)plotsd.c	3.11	1/18/97	ESI/ERL";
    ...
    char    **argv;
{
    ...
    long	maxpoints = 3000;   /* Approx. maximum no. of points to plot */
...
/* parse command line for parameters */
    ...
    while ((c = getopt(argc, argv, "r:p:s:t:x:y:E:M:Y:D")) != EOF)
	switch (c) 
        {
	    case 'r':
	    case 'p':
...
/* Find x-axis limits */
    ...
    if (h->common.ndrec != -1)	/* Input is file with fixed record size. */
	ndrec = h->common.ndrec;    /*  Get ndrec from header. */
    else			/* Input is pipe or has variable record size. */
	...
	do
	{
	    ...
	    if (num_read != 0)
		put_sd_recf(buf, num_read, tmphdr, tmpstrm);
	    ndrec += num_read;
	} while (num_read == BUFSIZE);
	Fclose(istrm);
	(void) rewind(tmpstrm);
	h = read_header(tmpstrm);
	istrm = tmpstrm;
    }
...
/* Read data. */
    ...
    fea_skiprec(istrm, (long) start_p - 1, h);
