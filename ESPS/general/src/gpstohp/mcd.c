...
static char *sccs_id = "@(#)mcd.c	1.2	10/4/91	ESI/ERL";

/*----------------------------------------------------------------------+
|									|
|   mcd - Sun filter for plot data in Unix GPS format			|
|									|
...
extern int label(), erase(), cont(), space();
int 
   line(), circle(),
arc(), move(),  point(), linemod(),
 closepl();
int             text();
int             plotline();
void            do_plot();
extern void     start_plot();
	...
	char          **argv;
{
	...
	Argc = argc;
	Argv = argv;	
        convert_options(Argv, Argc);
	init_device();

	while ((c = getopt(Argc, Argv, "T:r:ux:t:RPi:p:")) != EOF)
		switch (c) {
		case 'T':
			break;
		case 'e':
...
char **Argv;
int  Argc;
/* find -i xx and change it to -WI xx,
   and  -t xx and change it to -Wl xx   */

{
	int i;
	
	for (i=0; i<Argc; i++) {
		if (strcmp(Argv[i], "-i") == 0) Argv[i] = "-WI";
		if (strcmp(Argv[i], "-t") == 0) Argv[i] = "-Wl";
	}
	return;
}
