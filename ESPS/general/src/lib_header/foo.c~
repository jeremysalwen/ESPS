...
*/
...
#ifndef DEC_ALPHA
	if (!miio_get_long(&pre->machine_code, 1, YES, NONE, fd))
	  BADHEAD (lev, "error reading preamble");
	if (!miio_get_long(&pre->check_code, 1, YES, NONE, fd))
	  BADHEAD (lev, "error reading preamble");
	if (!miio_get_long(&pre->data_offset, 1, YES, NONE, fd))
	  BADHEAD (lev, "error reading preamble");
	if (!miio_get_long(&pre->record_size, 1, YES, NONE, fd))
	  BADHEAD (lev, "error reading preamble");
	if (!miio_get_long(&pre->check, 1, YES, NONE, fd))
	  BADHEAD (lev, "error reading preamble");
	if (!miio_get_long(&pre->edr, 1, YES, NONE, fd))
	  BADHEAD (lev, "error reading preamble");
	if (!miio_get_long(&pre->align_pad_size, 1, YES, NONE, fd))
	  BADHEAD (lev, "error reading preamble");
	if (!miio_get_long(&pre->foreign_hd, 1, YES, NONE, fd))
	  BADHEAD (lev, "error reading preamble");
#else
	if (!miio_get_int(&pre->machine_code, 1, YES, NONE, fd))
	  BADHEAD (lev, "error reading preamble");
	if (!miio_get_int(&pre->check_code, 1, YES, NONE, fd))
	  BADHEAD (lev, "error reading preamble");
	if (!miio_get_int(&pre->data_offset, 1, YES, NONE, fd))
	  BADHEAD (lev, "error reading preamble");
	if (!miio_get_int(&pre->record_size, 1, YES, NONE, fd))
	  BADHEAD (lev, "error reading preamble");
	if (!miio_get_int(&pre->check, 1, YES, NONE, fd))
	  BADHEAD (lev, "error reading preamble");
	if (!miio_get_int(&pre->edr, 1, YES, NONE, fd))
	  BADHEAD (lev, "error reading preamble");
	if (!miio_get_int(&pre->align_pad_size, 1, YES, NONE, fd))
	  BADHEAD (lev, "error reading preamble");
	if (!miio_get_int(&pre->foreign_hd, 1, YES, NONE, fd))
	  BADHEAD (lev, "error reading preamble");
#endif
	...
	if (pre->check != HD_CHECK_VAL) {
		if (EspsDebug) 
		  Fprintf(stderr,"no preamble here.\n");
...
*/
	  (void)bcopy((char *)pre, (char *)p, sizeof (*pre)); 
	  if (fread(((char *)p)+sizeof(*pre),1,FIXPART_LEN-sizeof(*pre), fd)==0)
...
/* if this is an older type file, then it is not EDR_ESPS */
	...
	else {	/* this is a valid preamble */
#ifdef MACH_CHECK_STUFF
	  if(pre->machine_code == MACH_CODE) {
		Fprintf(stderr,"This ESPS file was created on machine %s\n",
		  machine_codes[pre->machine_code]);
		Fprintf(stderr,
		  "and is not compatible with this machine (%s).\n",
		  machine_codes[MACH_CODE]);
		return NULL;	/* return NULL, which means error */
	  }
#endif

	  if (EspsDebug)
	  	Fprintf(stderr,"found preamble\n");
	  p->common.edr = pre->edr;
	  edr_flag = pre->edr;
	  p->common.machine_code = pre->machine_code;
	  machine_code = pre->machine_code;
	  if(EspsDebug)
...
#ifdef DEMO
	if (pre->check_code  != (3000 + hash(p->common.date)))
		return NULL;
#endif
	free((char *)pre);  /* we don't need this anymore */
    }
    else {
...
#ifndef DEC_ALPHA
    if ((p->common.machine_code != DEC_ALPHA_CODE) && (p -> common.fixpartsiz >= p -> common.hsize ||
...
/* We do not want to copy certain generic items in copy_header
   ...
   For now, fields "encoding" and "sphere_hd_ptr" are not to be copied.
*/
static int
ok_to_copy(name)
char *name;
{
    if (!name || strcmp(name,"encoding")==0 || strcmp(name,"sphere_hd_ptr")==0)
	return 0;
    else
	return 1;
}
...
*/
    ...
    if (get_genhd_val("sphere_hd_ptr", hd, (double)0.0)) {
