 ...
 *
 */
static char *sccs_id = "@(#)newplay.c	1.1 10/1/93 ERL";
...

{
  ...
  while((c = getopt(ac,av,"f:s:e:c:x:i:r:qhHwkRgC?")) != EOF) {
    switch(c) {
    case 'f':
      srate = atof(optarg);
...
/*	  if(debug_level) {
	    int dd;
	    double *dp = (double*)get_genhd("max_value",h->esps_hdr);
	    for(dd=0;dd<sig->dim;dd++)
