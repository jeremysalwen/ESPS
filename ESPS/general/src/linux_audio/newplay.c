...

{
  ...
  do {				/* for all input specifications... */
	...
	if(amp = (int)(*sig->smax > -(*sig->smin)? *sig->smax : -(*sig->smin))
	   < 128) amp = 32767;
    if(debug_level) {
		fprintf(stderr, "*sig->smax=%f, -(*sig->smin)=%f, amp set to %i\n",
			*sig->smax, -(*sig->smin), amp);
	    if(*sig->smax > -(*sig->smin)) fprintf(stderr,"%f > %f\n", *sig->smax,
			-(*sig->smin));
		else fprintf(stderr, "%f not > %f\n", *sig->smax, -(*sig->smin));
    ...
    if(debug_level)
      fprintf(stderr,
	      "Calling play_from_file. length=%d srate=%f channel=%d amp=%i\n",
