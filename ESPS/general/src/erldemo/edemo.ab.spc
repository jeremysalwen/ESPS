...
is for silence on all machines except for SPARCstations, which default
to spoken output using the internal speaker.

To select the desired output option, just push one of the mutually-
exclusive buttons in the lower left.  If you are running on a
SPARCstation, the best sound quality is obtained if an external
